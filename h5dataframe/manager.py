...
from pandas.core.internals.base import ensure_np_dtype, interleaved_dtype
from pandas.core.internals.blocks import ensure_block_shape, external_values
...
class H5ArrayManager(BaseArrayManager):
    def __init__(
        self,
        ...
        self.typ = "file" if any(isinstance(arr, ch.H5Array) for arr in arrays) else "ram"
    ...
    def insert(self, loc: int, item: Hashable, value: NDArrayLike[Any], refs=None) -> None:
        """
    ...
    def idelete(self, indexer) -> H5ArrayManager:
        """
    ...
    def grouped_reduce(self, func: Callable) -> H5ArrayManager:
        """
    ...
    def reduce(self, func: Callable) -> H5ArrayManager:
        """
        ...
        index = pd.Index._simple_new(np.array([None], dtype=object))  # placeholder
        columns = self.items
    ...
    # ----------------------------------------------------------------
    def unstack(self, unstacker, fill_value) -> H5ArrayManager:
        """
    ...
    @classmethod
    def concat_horizontal(cls, mgrs: list[H5ArrayManager], axes: list[pd.Index]) -> H5ArrayManager:
        """
    ...
    @classmethod
    def concat_vertical(cls, mgrs: list[H5ArrayManager], axes: list[pd.Index]) -> H5ArrayManager:
        """
...
class H5SingleArrayManager(BaseArrayManager, SingleDataManager):
    __slots__ = [
    ...
    def __init__(
        ...
        self.typ = "file" if isinstance(arrays[0], ch.H5Array) else "ram"

        if verify_integrity:
            assert len(axes) == 1
            assert len(arrays) == 1
            self._axes = [ensure_index(ax) for ax in axes]
            self.arrays = [extract_pandas_array(arrays[0], self.typ)]
            self._verify_integrity()
    ...
    def make_empty(self, axes=None) -> H5SingleArrayManager:
    ...
    @classmethod
    def from_array(cls, array, index) -> H5SingleArrayManager:
        return cls([array], [index])

    # error: Cannot override writeable attribute with read-only property
    @property
    def axes(self) -> list[pd.Index]:  # type: ignore[override]
        return self._axes
    ...
    def apply(self, func, **kwargs) -> H5SingleArrayManager:  # type: ignore[override]
        if callable(func):
    ...
    def setitem(self, indexer, value, warn: bool = True) -> H5SingleArrayManager:
        """
    ...
    def _get_data_subset(self, predicate: Callable) -> H5SingleArrayManager:
    ...
    def set_values(self, values: NDArrayLike[Any]) -> None:
        ...
        """
        self.typ = "file" if isinstance(values, ch.H5Array) else "ram"
        self.arrays[0] = values

    def to_2d_mgr(self, columns: pd.Index) -> H5ArrayManager:
        """
        ...
        return H5ArrayManager(arrays, axes, typ=self.typ, verify_integrity=False)
